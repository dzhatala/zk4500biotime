...
public class MainWin {
	...
	public static void main(String[] args) {
		...
		try{  
			...
			ans = cs.readLine();
			if(ans!=null&&ans.length()>0)database=ans;
	 ...
	 static void speakNotRegistered() {
		...
		Thread runner =new Thread(){
			...
			public void run(){
				try {
					Process p=Runtime.getRuntime().exec("vlcwin.bat notregistered.wav");
					p.waitFor();
					//Thread.currentThread();
					//Thread.sleep(1000);
					p=Runtime.getRuntime().exec("vlcwin.bat  tdk_terdaftar.wav");
					p.waitFor();
					p=Runtime.getRuntime().exec("vlcwin.bat silahkan_ulangi.wav");
					p.waitFor();
				} catch (IOException e) {
		...
		Thread runner =new Thread(){
			...
			public void run(){
				try {
					...
					Process p=Runtime.getRuntime().exec("vlcwin.bat notindb.wav");
					p.waitFor();
					...
					/*if(true==true){
						...
						p.waitFor();
						p=Runtime.getRuntime().exec("vlcwin.bat  fpid.wav");
						p.waitFor();
					}*/
					p=Runtime.getRuntime().exec("vlcwin.bat  tdk_basis_data.wav");
					p.waitFor();
					p=Runtime.getRuntime().exec("vlcwin.bat  silahkan_ulangi.wav");
					p.waitFor();
				} catch (IOException e) {
	...
	private static void speakIdentifed1N_notused(int ffPID, final String ffName, final String flName) {
		// TODO Auto-generated method stub
		...
		Thread runner =new Thread(){
			...
			public void run(){
				...
				//Runtime.getRuntime().exec(cmd);
					...
					p.waitFor();
					p=Runtime.getRuntime().exec("vlcwin.bat test.wav");
					p.waitFor();
					...
					if(ffName!=null){
						...
						p=Runtime.getRuntime().exec(args);
						p.waitFor();
						p=Runtime.getRuntime().exec("vlcwin.bat fN.wav");
						p.waitFor();
					}
	...
	public static void speakIdentifed1N_espeak(int ffPID, final String ffName, final String flName) {
		// TODO Auto-generated method stub
		System.out.println("speakIdentifed1N_espeak ...");
		Thread runner =new Thread(){
			...
			public void run(){
				try {
					Process p=null;
					String []args=null;
					//p=Runtime.getRuntime().exec("vlcwin.bat  verified.wav");
					p=Runtime.getRuntime().exec("vlcwin.bat  verified.wav");
					p.waitFor();
					//Thread.currentThread();
					//Thread.sleep(500);
					...
					if(ffName!=null){
						...
						//args[0]="espeak";args[1]="-p"; args[2]="50";args[3]= "-v";args[4]="mb-id1"; args[5]="-w";
						//args[6]="espeak.wav";
						//args[7]=ffName;
